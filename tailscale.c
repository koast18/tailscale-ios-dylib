...
extern int TsnetSetProxy(int sd, char* proxyURL);
extern int TsnetSetDisableP2P(int sd, int disable);
...
int tailscale_set_proxy(tailscale sd, const char* proxy_url) {
	return TsnetSetProxy(sd, (char*)proxy_url);
}

int tailscale_set_disable_p2p(tailscale sd, int disable) {
	return TsnetSetDisableP2P(sd, disable);
}

int tailscale_set_param(tailscale sd, const char* key, const char* value) {
	return TsnetSetParam(sd, (char*)key, (char*)value);
}

int tailscale_get_param(tailscale sd, const char* key, char* buf, size_t buflen) {
	return TsnetGetParam(sd, (char*)key, buf, buflen);
}

int tailscale_get_status_json(tailscale sd, char* buf, size_t buflen) {
	return TsnetGetStatusJSON(sd, buf, buflen);
}
...
int tailscale_edit_prefs_json(tailscale sd, const char* json) {
	return TsnetEditPrefsJSON(sd, (char*)json);
}
