...
libtailscale_ios.a:
	...
	GOOS=ios GOARCH=arm64 CC=$(PWD)/scripts/clangwrap-ios.sh go build -v -ldflags -w -tags ios -o $@ -buildmode=c-archive

libtailscale_ios_sim_arm64.a:
	GOOS=ios GOARCH=arm64 CC=$(PWD)/scripts/clangwrap-ios-sim-arm.sh go build -v -ldflags -w -tags ios -o $@ -buildmode=c-archive

libtailscale_ios_sim_x86_64.a:
	GOOS=ios GOARCH=amd64 CC=$(PWD)/scripts/clangwrap-ios-sim-x86.sh go build -v -ldflags -w -tags ios -o $@ -buildmode=c-archive

libtailscale_ios_sim.a: libtailscale_ios_sim_arm64.a libtailscale_ios_sim_x86_64.a
	lipo -create -output $@ libtailscale_ios_sim_x86_64.a libtailscale_ios_sim_arm64.a

.PHONY: c-archive-ios
c-archive-ios: libtailscale_ios.a  ## Builds libtailscale_ios.a for iOS (iOS SDK required)

.PHONY: c-archive-ios-sim
c-archive-ios-sim: libtailscale_ios_sim.a ## Builds a fat binary for iOS (iOS SDK required)
...
.PHONY: ios-dylib
ios-dylib: ## Builds iOS device/simulator dylibs plus macOS dylib (macOS + Xcode required)
	./scripts/build-ios-dylib.sh
